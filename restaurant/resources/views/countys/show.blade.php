...
@section('content')
		...
		<div class="card-body">
			...
			@auth
				@if(Illuminate\Support\Facades\Auth::user()->id === $county->admin->id)
					<div class="actions">
						<a href="{{ route('countys.create') }}" class="btn btn-dark">Create new county</a>
						<a href="{{ route('countys.edit', ['county' => $county]) }}" class="btn btn-success">Edit county</a>

						<form action="{{ route('countys.destroy', ['county' => $county]) }}" method="POST">
							@csrf
							@method('DELETE')
	...
	<div class="mt-4">
		<a href="{{ route('countys.index') }}" class="btn btn-dark">Back to countys</a>
	</div>
@endsection